...
use App\Jobs\Job;
use App\Mail\V1\Auth\VerifyCodeMail;
use App\Services\Api\V1\Auth\CodeService;
use Carbon\Carbon;
use Illuminate\Support\Facades\Mail;

final class SendCodeMailJob extends Job
{
    ...
    public function handle(CodeService $codeService): void
    {
        if ($code = $codeService->get(['uuid' => $this->codeUuid])) {
            Mail::to($this->email)->send(new VerifyCodeMail($code->getAccountUuid(), $this->email, $code->getCode()));
