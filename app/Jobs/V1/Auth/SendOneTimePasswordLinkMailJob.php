...
use App\Jobs\Job;
use App\Mail\V1\Auth\OneTimePasswordLink;
use App\Services\Api\V1\Auth\CodeService;
use Carbon\Carbon;
use Illuminate\Support\Facades\Mail;

final class SendOneTimePasswordLinkMailJob extends Job
{
    ...
    public function handle(CodeService $codeService): void
    {
        if ($code = $codeService->get(['uuid' => $this->codeUuid])) {
            Mail::to($this->email)->send(new OneTimePasswordLink($code->getAccountUuid(), $this->email, $code->getCode()));
